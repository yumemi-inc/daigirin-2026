...
| ステップ | 内容 |
| :-: | :-- |
| **1** | Anchor は Solana プログラム開発のボイラープレートを吸収するフレームワークであり、本質的なロジックに集中できる |
| **2** | `#[program]` で Instruction を定義し、`#[derive(Accounts)]` で必要なアカウント一覧を宣言するのが基本パターン |
| **3** | PDA Mint の authority を PDA 自身に設定することで、プログラムからのみ Mint/Burn が可能になる |
| **4** | CPI の signer_seeds により PDA の所有証明を行い、Token Program への委譲が安全に動作する |
| **5** | 独自データ構造（PlayerData PDA）により、プレイヤーごとの状態管理をオンチェーンで実現できる |
...
- Anchor フレームワークで Solana プログラム開発に入門する <span class="footnote">https://zenn.dev/acntechjp/articles/a40aba558a89d4</span>
