...
「Bool で nil は false として扱えばいい」と考える人もいるでしょう。しかし、なぜ false と判定したのでしょうか。

この判断基準は、言語によって異なる場合があります。それが「Truthiness」という概念です。

### Truthiness（真偽値性）とは

プログラミングにおいて、true/false そのものではない値（数値、文字列、オブジェクトなど）が、条件式（if文など）の中で「真」または「偽」として扱われる性質を Truthiness（真実味）と呼びます。

「何が真か」を覚えるより、「何が偽（Falsy）とされるか」を覚える方が簡単です。それ以外はすべて「真（Truthy）」になるからです。このルールを知っておくと、コードを短く書ける一方で、思わぬバグの原因にもなります。
...
注意点とし、 `[]`（空の配列）や `{}`（空のオブジェクト）は「真（Truthy）」です。ここが他の言語と異なり、よくハマるポイントです。
...
言語によって Falsy の定義が異なるため、どの言語を利用しているかによって動作が変わります。また、コードを書くエンジニアがどの言語をバックボーンとするか、そもそものエンジニアの性格などで、Falsy に対する意識は異なります。これが、nil の扱いを難しくしている要因の１つです。

## nil に意味を持たせる危険性

さらに厄介なのは nil 状態に意味を持たせた場合です。例えば、新機能があり、ユーザーに利用許諾を取ってから実行する場合を考えてみましょう。
...
このコードは、Boolに「まだリクエストしていない」という第三の状態を付けてしまっています。一見、言語仕様にしたがって Bool を Optional で拡張して効率的に見えますが、これは事故です。

### Optional Bool の問題性

この問題があるコード例は nil を利用許諾するという目的の初期値として利用しています。一般に Bool は二値という共通認識があるため、nil が初期状態（未リクエスト）という考えは、紐付きません。

Bool は二値のための型です。nil に、nil 以上の意味を付けるのは絶対にやめましょう。

### 三状態以上の適切なアプローチ

この場合は、Enumで三状態を明示的に定義しましょう。
