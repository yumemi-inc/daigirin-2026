...
プログラミング言語において、Bool 型は基本的なデータ型の１つです。二値を表現するシンプルな型です。また、モダンな言語は nil 安全を採用して、型レベルで nil を制御できるようになりました。この２つが混じると、コードの可読性が下がり、バグの温床になりえます。
...
注意点として、 `[]`（空の配列）や `{}`（空のオブジェクト）は「真（Truthy）」です。ここが他の言語と異なり、よく誤るポイントです。
